    ...
    methods: {
        ...
        async loadProduct() {
            try {
                ...
                // 방법 1. {params:{page:10, size:2}} 와 같은 형식으로 전송 시 parameter 형식으로 변환되어 서버로 전송
                // 방법 2. formData 객체 생성하여 서버로 전송
                let params = {
                    size: this.pageSize,
        ...
        addCart() {
            const orderProducts = Object.keys(this.selected)
                ...
                .map(key => {
                    ...
                    return { productId: product.id, name: product.name, quantity: product.quantity };
                });
        ...
        async createOrder() {
            const orderProducts = Object.keys(this.selected)//  객체의 모든 키를 배열로 반환
                .filter(key => this.selected[key])//    selected 객체의 값이 true인 키만 필터링
                .map(key => {
                    const product = this.productList.find(p => p.id == key);
